...
builder.Host
    ...
    {
        var infrastructureAssembly = typeof(GymWriteDbContext).Assembly;
...
{
    options.SwaggerDoc("v1", new() { Title = "Gym Rest API", Version = "v1" });
});

var connectionString = builder.Configuration.GetConnectionString("DefaultConnection");
builder.Services.AddDbContextPool<GymWriteDbContext>(i => { i.UseNpgsql(connectionString); });
builder.Services.AddDbContextPool<GymReadDbContext>(i => { i.UseNpgsql(connectionString); });
...
var dbContext = createScope.ServiceProvider.GetService<GymWriteDbContext>();

if (app.Environment.IsDevelopment())
{
    app.UseDeveloperExceptionPage();
    app.UseSwagger();
    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Gym Rest API v1"));
}
